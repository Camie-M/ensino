...
export const getAllUsers = async (): Promise<UserInfoProp[] | null> => {
    try {
        const usersResponse = await fetch('http://192.168.15.18:3001/users', {
            method: "GET",
...
try {
    ...
    const response = await fetch(`http://192.168.15.18:3001/users`, {
    method: "POST",
    headers: {
        'Authorization': `${token}`,
    },
...
export const getUserById = async (id: string): Promise<UserInfoProp | null> => {
    try {
        const userResponse = await fetch(`http://192.168.15.18:3001/users/${id}`, {
            method: "GET",
            headers: { "Content-Type": "application/json" },
        });
...
export const updateUserbyId = async (id: string, formData: UserInfoProp): Promise<UserInfoProp | null> => {
  try {
    const token = await AsyncStorage.getItem('userToken');
    const formatedFormData = await formatFormData(formData)
    const response = await fetch(`http://192.168.15.18:3001/users/${id}`, {
      method: "PUT",
      headers: {
        'Authorization': `${token}`,
      },
